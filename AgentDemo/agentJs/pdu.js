...
function hton8Int(buff, val, pos) {
  var i8View = new DataView(buff);
  if (val < 0) {
      i8View.setInt8(pos, val, big);
   }
   else {
      i8View.setUint8(pos, val, big);
   }
   return pos + Uint8Array.BYTES_PER_ELEMENT;
}
function hton16(buff, val, pos) {    //小端存储
   var i16NetView = new DataView(buff);
   if (val < 0) {
      i16NetView.setInt16(pos, val, little);
   }
   else {
      i16NetView.setUint16(pos, val, little);
   }
   return pos + Uint16Array.BYTES_PER_ELEMENT;
}

function hton16(buff, val, pos) { //大端存储
   var i16NetView = new DataView(buff);
   if (val < 0) {
      i16NetView.setInt16(pos, val, big);
   }
   else {
      i16NetView.setUint16(pos, val, big);
   }
   return pos + Uint16Array.BYTES_PER_ELEMENT;
}

function hton32(buff, val, pos) {    //小端存储
   var i32NetView = new DataView(buff);
   if (val < 0) {
      i32NetView.setInt32(pos, val, little);
   }
   else {
      i32NetView.setUint32(pos, val, little);
   }
   
   return pos + Uint32Array.BYTES_PER_ELEMENT;
}

function htobn32(buff, val, pos) {    //小端存储
   var i32NetView = new DataView(buff);
   if (val < 0) {
      i32NetView.setInt32(pos, val, big);
   }
   else {
      i32NetView.setUint32(pos, val, big);
   }
...
function ntoh8Int(buff, pos) {
    var dv = new DataView(buff);
    var res = dv.getInt8(pos, little);
    return [res, pos+Int8Array.BYTES_PER_ELEMENT];
}
function ntoh16(buff, pos) {
    var dv = new DataView(buff);
    var res = dv.getInt16(pos, little);
    return [res, pos+Int16Array.BYTES_PER_ELEMENT];
}

function ntoh32(buff, pos) {  //小端
    var dv = new DataView(buff);
    var res = dv.getInt32(pos, little);
    return [res, pos+Int32Array.BYTES_PER_ELEMENT];
}

function ntobh32(buff, pos) {
    var dv = new DataView(buff);
    var res = dv.getInt32(pos, big);
    return [res, pos+Int32Array.BYTES_PER_ELEMENT];
}

function ntoh32f(buff, pos) {
    var dv = new DataView(buff);
    var res = dv.getFloat32(pos, little);
...
function SetAgentStatusPdu(eventID,agentID,agentGroupID,deviceAddress,state,password,description)
{
	...
	var prevEndPos = htobn32(buff, buff.byteLength-MSG_HEADER,10);
	...
	prevEndPos = hton8Int(buff, state, prevEndPos);
	prevEndPos = hton8(buff, password, prevEndPos,MAX_PASSWORD+1);
	//var b=prevEndPos;
	description=uniencode(description);//对中文汉字进行编码
	prevEndPos = hton8(buff, description, prevEndPos, MAX_DESCRIPTION+1);
	//var res=ntoh8(buff,b,MAX_DESCRIPTION+1);
	//logger1.log("destination:"+decodeURI(decodeURI(res[0],"UTF-8")));
	return buff;
...
function TransferCallPdu(eventID,agentID,callID,deviceAddress,destination,applicationData,isBlind,timeout,priority)
{
   ...
   var buff = getArrayBuff(["UINT8", 10], 
	...
	prevEndPos = hton8(buff, destination, prevEndPos, MAX_ADDRESS+1);
	prevEndPos = hton8(buff, applicationData, prevEndPos, MAX_APPDATA+1);
	prevEndPos = hton8Int(buff, isBlind, prevEndPos, 1);
...
function TransferToAgentPdu(eventID,agentID,callID,deviceAddress,applicationData,timeout)
{
	var buff = getArrayBuff(["UINT8", 10], 
	["UINT32", 1], 
	["UINT32", 1], 
...
function AgentStatusPdu(eventID,agentName,agentID,deviceAddress,loginTime,state,cause)
{
	...
	prevEndPos = hton32(buff, state, prevEndPos);
...
function CallDetailPdu(eventID,callingNumber,calledNumber,agentID,fromRoutePoint,toRoutePoint,beginTime,pickupTime,endTime,isInbound,enumcallID,isAgentHangupFirst)
{
	...
	prevEndPos = hton8Int(buff, isInbound, prevEndPos);
	prevEndPos = hton8(buff, enumcallID, prevEndPos,MAX_CALLID_LEN+1);
	hton8Int(buff, isAgentHangupFirst, prevEndPos);
...
function OnTimerPdu (eventID,timerEventID,data)
{
	...
	prevEndPos = hton8(buff, data, prevEndPos,data.length);	
	return buff;	
}
function LeaveCausesPdu (eventID,count,data)
{
	...
	prevEndPos = hton8INt(buff, data, prevEndPos,1);	
	return buff;
}

function	LeaveCause (causeID,cause)
{
	var buff = getArrayBuff(["UINT8", 10], 
	["UINT32", 1], 
	["UINT32", 1], 
...
function TraceSettingPdu (eventID,traceOn,traceSetting)
{
	...
	var prevEndPos = htobn32(buff, buff.byteLength-MSG_HEADER,10);
	prevEndPos = hton32(buff, eventID,prevEndPos);
	prevEndPos = hton8Int(buff, traceOn, prevEndPos);
...
function RouteRequestPdu(eventID,callID,routeRegisterReqID,routingCrossRefID,routepoint,calling)
{
	...
	hton8(buff, calling,prevEndPos,MAX_ADDRESS+1);
	return buff;
}
...
function ServiceAgentResultPdu (eventID,requestID,isAccept)
{
	...
	prevEndPos = hton32(buff, eventID,prevEndPos);
	prevEndPos = hton32(buff, requestID,prevEndPos);
	hton8Int(buff, isAccept,prevEndPos);
	return buff;
}
...
function AutoMakeCallPdu (eventID,agentID,callID,destination,timeout,tipInfo)
{
	var buff = getArrayBuff(["UINT8", 10], 
	["UINT32", 1], 
	["UINT32", 1],  
...
callState,
callType,
originalCallingPartyType,
originalCallingParty,
originalCalledPartyType,
originalCalledParty,
currentCallingPartyType,
currentCallingParty,
currentCalledPartyType,
currentCalledParty,
monitorPartyType,
monitorParty,
dnis,   
...

{
    ...
    write_delimiter(buff);
    var prevEndPos = htobn32(buff, 6666,10);
    prevEndPos = hton32(buff, eventID, prevEndPos);
    ...
    prevEndPos = hton64(buff,device, prevEndPos);
    ...
    prevEndPos = hton8(buff, originalCallingParty, prevEndPos, MAX_ADDRESS+1);
    prevEndPos = hton32(buff, originalCalledPartyType, prevEndPos);
    ...
    prevEndPos = hton8(buff, applicationData, prevEndPos, MAX_APPDATA+1);
 	...
 	prevEndPos = hton64(buff, functiona, prevEndPos);
 	prevEndPos = hton32(buff, asyncHandle, prevEndPos);
 	prevEndPos = hton32(buff, errorCode, prevEndPos);
 	prevEndPos = hton8(buff, enumcallID, prevEndPos, MAX_CALLID_LEN+1);
    ...
    prevEndPos = hton8(buff, queueName, prevEndPos, MAX_QUEUE_LEN+1);   
	prevEndPos = hton32(buff, queueMembers, prevEndPos);
	prevEndPos = hton32(buff, memberPostion, prevEndPos);
	prevEndPos = hton32(buff, holdTime, prevEndPos);
	prevEndPos =hton32(buff, dtmf, prevEndPos);
	prevEndPos = hton8(buff, pbxUniqueID, prevEndPos, MAX_CALLID_LEN+1);
    hton8(buff, leaveQueueCause, prevEndPos, MAX_CAUSE_LEN+1);
